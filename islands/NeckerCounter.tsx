...
export default function NeckerCounter(
  { csrfToken, canIncrease = true, canDecrease = true }: NeckerCounterProps,
) {
  ...
  return (
    <div
      class={`border-2 rounded-lg p-4 max-[380px]:p-3 sm:p-4 lg:p-3.5 transition-colors ${
        isLow
    ...
    >
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-xs max-[380px]:text-[10px] mt-1 opacity-70">
            in stock
          </p>
          {!saving.value && error.value && (
            <p class="text-xs max-[380px]:text-[10px] text-red-500 mt-1">
              {error.value}
            </p>
          )}
        </div>
        <div
          class="text-4xl max-[380px]:text-3xl sm:text-3xl lg:text-[1.9rem]"
          aria-hidden="true"
        >
          🧣
        </div>
