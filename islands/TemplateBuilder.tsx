...
export default function TemplateBuilder(
  { allItems, csrfToken }: TemplateBuilderProps,
) {
  ...
  const locationFilter = useSignal("all");
  ...
  const selectedIds = useComputed(() =>
    new Set(selectedItems.value.map((i) => i.itemId))
  );

  const availableItems = useComputed(() =>
    allItems.filter((inv) => !selectedIds.value.has(inv.id))
  );

  const availableCategories = useComputed(() =>
    new Set(availableItems.value.map((inv) => inv.category))
  );

  const availableCategoryGroups = useComputed(() => {
    const available = availableCategories.value;
    ...
    const groups: Array<{ label: string; categories: ItemCategory[] }> = [];
    ...
    if (other.length > 0) {
      groups.push({ label: "Other", categories: other as ItemCategory[] });
    }
    return groups;
  });
  ...
  const availableLocationGroups = useComputed(() => {
    const uniqueLocations = [
      ...new Set(availableItems.value.map((inv) => inv.location)),
    ];
  ...
  const filteredItems = useComputed(() => {
    ...
    return availableItems.value
      .filter((inv) =>
        categoryFilter.value === "all" || inv.category === categoryFilter.value
      )
      .filter((inv) =>
        locationFilter.value === "all" || inv.location === locationFilter.value
      )
      .filter((inv) =>
  ...
  async function save() {
    ...
    if (!trimmedName) {
      error.value = "A template name is required.";
      return;
    }
            ...
            <div class="space-y-2">
              ...
              <div class="grid grid-cols-1 md:grid-cols-3 gap-2">
                ...
                >
                  ...
                  {availableCategoryGroups.value.map((group) => (
