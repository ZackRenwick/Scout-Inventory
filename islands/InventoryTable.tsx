...
function getContentsSummary(item: InventoryItem):
  | { entries: number; totalQuantity: number }
  | null {
  const contents =
    (item as { contents?: Array<{ quantity?: number }> }).contents;
  if (!Array.isArray(contents) || contents.length === 0) {
    return null;
  }
  const entries = contents.length;
  const totalQuantity = contents.reduce((sum, entry) => {
...
export default function InventoryTable(
  ...
  const categoryFilter = useSignal<"all" | ItemCategory>(
    initialCategory as "all" | ItemCategory,
  );
  const spaceFilter = useSignal<
    "all" | "camp-store" | "scout-post-loft" | "gas-storage-box"
  >("all");
  ...
  const sortField = useSignal<"name" | "location" | "status" | "quantity">(
    "name",
  );
  ...
  const filteredItems = useComputed(() => {
    const filtered = itemsSignal.value.filter((item) => {
      ...
      if (showNeedsRepair.value) {
        ...
        const condNeedsRepair = hasCondition &&
          (item as { condition: string }).condition === "needs-repair";
        const partialRepair =
          (item as { quantityNeedsRepair?: number }).quantityNeedsRepair ?? 0;
      ...
      if (cookingTypeFilter.value !== "all") {
        ...
        if (
          (item as { equipmentType?: string }).equipmentType !==
            cookingTypeFilter.value
        ) {
          return false;
        }
      }
      ...
      const anyFoodExpiry = showExpiredFood.value || showExpiringSoon.value ||
        showExpiring30.value;
      if (anyFoodExpiry) {
        if (!isFoodItem(item)) {
          return false;
        }
    ...
    return [...filtered].sort((a, b) => {
      ...
      if (sortField.value === "status") {
        const score = (item: InventoryItem) => {
          ...
          if (
            ((item as { quantityNeedsRepair?: number }).quantityNeedsRepair ??
              0) > 0
          ) return 1;
          ...
          if (
            "condition" in item &&
            (item as { condition: string }).condition === "good"
          ) return 6;
          return 7;
        };
  ...
  return (
    <div>
      ...
      {toast.value && (
        <div
          class={`fixed top-4 right-4 z-50 px-5 py-3 rounded-lg shadow-lg text-white font-medium transition-all ${
      ...
      <div class="bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 shadow-sm mb-6">
          ...
          <div class="flex items-center gap-2">
            {atCampCount.value > 0 && (
              <button
                type="button"
                onClick={() => {
                  showAtCamp.value = !showAtCamp.value;
                }}
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 p-4 pb-3">
          ...
          <div>
            ...
            <select
              ...
              onChange={(e) =>
                categoryFilter.value = (e.target as HTMLSelectElement)
            ...
            >
              ...
              <optgroup label="🏪 Camp Store">
                <option value="tent">
                  {getCategoryEmoji("tent")} {getCategoryLabel("tent")}
                </option>
                <option value="cooking">
                  {getCategoryEmoji("cooking")} {getCategoryLabel("cooking")}
                </option>
                <option value="food">
                  {getCategoryEmoji("food")} {getCategoryLabel("food")}
                </option>
                <option value="camping-tools">
                  {getCategoryEmoji("camping-tools")}{" "}
                  {getCategoryLabel("camping-tools")}
                </option>
              </optgroup>
              <optgroup label="🏠 Scout Post Loft">
                <option value="games">
                  {getCategoryEmoji("games")} {getCategoryLabel("games")}
                </option>
              </optgroup>
              <optgroup label="🛢️ Gas Storage Box">
                <option value="fuel">
                  {getCategoryEmoji("fuel")} {getCategoryLabel("fuel")}
                </option>
              </optgroup>
            </select>
          </div>
          {(categoryFilter.value === "all" ||
            categoryFilter.value === "cooking") && (
            <div>
              <label
                htmlFor="cooking-type-filter"
                class="block text-xs font-medium text-gray-500 dark:text-gray-400 mb-1"
              >
                Equipment Type
              </label>
          ...
          <div
            class={(categoryFilter.value === "all" ||
                categoryFilter.value === "cooking")
              ? ""
              : "sm:col-span-2"}
          >
        ...
        <div class="flex flex-col gap-2 px-4 py-3 border-t border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50 rounded-b-lg">
          ...
          <div class="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-x-8 gap-y-2">
            <span class="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide px-3 sm:border-r border-gray-300 dark:border-gray-600 shrink-0">
              Stock
            </span>
            <label class="flex items-center gap-2 cursor-pointer select-none w-full sm:w-auto">
              <input
                ...
                onChange={(e) =>
                  showLowStock.value = (e.target as HTMLInputElement).checked}
            ...
            <label class="flex items-center gap-2 cursor-pointer select-none w-full sm:w-auto">
              <input
                ...
                onChange={(e) =>
                  showAtCamp.value = (e.target as HTMLInputElement).checked}
              ...
              {atCampCount.value > 0 && (
                <span class="text-xs font-medium px-1.5 py-0.5 rounded-full bg-amber-100 text-amber-800 dark:bg-amber-900/40 dark:text-amber-300">
                  {atCampCount.value}
                </span>
              )}
            </label>
            <label class="flex items-center gap-2 cursor-pointer select-none w-full sm:w-auto">
              <input
                ...
                onChange={(e) =>
                  showOnLoan.value = (e.target as HTMLInputElement).checked}
              ...
              {loanedCount > 0 && (
                <span class="text-xs font-medium px-1.5 py-0.5 rounded-full bg-blue-100 text-blue-800 dark:bg-blue-900/40 dark:text-blue-300">
                  {loanedCount}
                </span>
              )}
            </label>
          ...
          <div class="flex flex-col sm:flex-row sm:flex-wrap sm:items-center gap-x-8 gap-y-2">
            ...
            <label class="flex items-center gap-2 cursor-pointer select-none w-full sm:w-auto">
              <input
                ...
                onChange={(e) =>
                  showExpiring30.value = (e.target as HTMLInputElement).checked}
        ...
        {filteredItems.value.length === 0
          ...
          : (
            filteredItems.value.map((item) => {
              ...
              const partialRepairQty =
                (item as { quantityNeedsRepair?: number })
                  .quantityNeedsRepair ?? 0;
              const needsRepair = ("condition" in item &&
                (item as { condition: string }).condition ===
                  "needs-repair") || partialRepairQty > 0;
              ...
              return (
                <div
                  key={item.id}
                  class={`bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 border-l-4 ${accentColor} shadow-sm`}
                >
                  {/* Card header */}
                  <div class="flex items-start justify-between px-4 pt-4 pb-2">
                    ...
                    <div class="flex items-center gap-1.5 shrink-0 ml-2">
                      {(item as { atCamp?: boolean }).atCamp && (
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-amber-100 text-amber-800 border-amber-300 dark:bg-amber-900/40 dark:text-amber-300 dark:border-amber-600">
                          🏕️ At Camp
                        </span>
                      )}
                      {isLowStock && (
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-red-100 text-red-700 border-red-300 dark:bg-red-900/40 dark:text-red-300 dark:border-red-700">
                          ⚠️ Low
                        </span>
                      )}
                      {partialRepairQty > 0 && (
                        <span class="inline-flex items-center px-2 py-0.5 rounded-full text-xs font-medium border bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-700">
                          🔧 {partialRepairQty} need repair
                        </span>
                      )}
                    </div>
                  </div>
                  ...
                  <div class="grid grid-cols-2 gap-x-4 gap-y-1 px-4 pb-3 text-sm">
                    <div>
                      ...
                      {canEdit
                        ? (
                          <div class="flex items-center gap-1.5 mt-0.5">
                            ...
                            <span class="font-semibold text-gray-900 dark:text-gray-100 min-w-[1.5rem] text-center">
                              {item.quantity}
                            </span>
                            <button
                              type="button"
                      ...
                      <span class="text-gray-300 dark:text-gray-400 text-xs">
                      ...
                      {!!(item as { quantityAtCamp?: number }).quantityAtCamp &&
                        (
                          <span class="block text-xs text-amber-600 dark:text-amber-400 mt-0.5">
                            🏕️ {(item as { quantityAtCamp?: number })
                              .quantityAtCamp} at camp
                          </span>
                        )}
                    </div>
                    <div>
                    ...
                    {contentsSummary && (
                      <div class="col-span-2 pt-1">
                        <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-xs font-medium bg-purple-50 text-purple-700 dark:bg-purple-900/25 dark:text-purple-300">
                          📦 Contents: {contentsSummary.totalQuantity}{" "}
                          ({contentsSummary.entries}{" "}
                          {contentsSummary.entries === 1 ? "item" : "items"})
                        </span>
                      </div>
                    )}
                    {isFoodItem(item) && (
          ...
          <thead class="bg-gray-50 dark:bg-gray-800">
            <tr>
              <th
                ...
                onClick={() => {
                  if (sortField.value === "name") {sortDir.value =
                      sortDir.value === "asc" ? "desc" : "asc";} else {
                    sortField.value = "name";
                    sortDir.value = "asc";
                  }
                }}
              >
                Item {sortField.value === "name"
                  ? (sortDir.value === "asc" ? "↑" : "↓")
              ...
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
                onClick={() => {
                  if (sortField.value === "quantity") {sortDir.value =
                      sortDir.value === "asc" ? "desc" : "asc";} else {
                    sortField.value = "quantity";
                    sortDir.value = "asc";
                  }
                }}
              >
                Quantity {sortField.value === "quantity"
                  ? (sortDir.value === "asc" ? "↑" : "↓")
                  : <span class="opacity-30">↕</span>}
              </th>
              <th
                class="px-6 py-3 text-left text-xs font-medium text-gray-500 dark:text-gray-400 uppercase tracking-wider cursor-pointer select-none hover:text-gray-700 dark:hover:text-gray-200"
                onClick={() => {
                  if (sortField.value === "location") {sortDir.value =
                      sortDir.value === "asc" ? "desc" : "asc";} else {
                    sortField.value = "location";
                    sortDir.value = "asc";
                  }
                }}
              >
                Location {sortField.value === "location"
                  ? (sortDir.value === "asc" ? "↑" : "↓")
                  : <span class="opacity-30">↕</span>}
              </th>
              <th
                ...
                filteredItems.value.map((item) => (
                  <>
                    <tr
                      ...
                      onClick={() => {
                        globalThis.location.href = `/inventory/${item.id}`;
                      }}
                    >
                      {(() => {
                        const contentsSummary = getContentsSummary(item);
                        ...
                        <div class="flex flex-col items-start gap-1">
                          ...
                          {isTentItem(item) && (
                            <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border bg-blue-100 text-blue-800 border-blue-300 dark:bg-blue-900/40 dark:text-blue-300 dark:border-blue-700">
                              👤 {item.capacity}/tent · 🏕️{" "}
                              {item.capacity * item.quantity} total
                            </span>
                          )}
                          {isFoodItem(item) && (
                            <ExpiryBadge expiryDate={item.expiryDate} />
                          ...
                          {("quantityNeedsRepair" in item) &&
                            ...
                            (
                              <span class="inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border bg-yellow-100 text-yellow-800 border-yellow-300 dark:bg-yellow-900/40 dark:text-yellow-300 dark:border-yellow-700">
                                🔧 {(item as { quantityNeedsRepair?: number })
                                  .quantityNeedsRepair} need repair
                              </span>
                            )}
                          {"condition" in item &&
                            (item as { condition: string }).condition !==
                            ...
                            (() => {
                              const cond =
                                (item as { condition: string }).condition;
                              const condLabel = cond === "excellent"
                                ? "✅ Excellent"
                                : cond === "good"
                                ? "✅ Good"
                                : "⚠️ Fair";
                              ...
                              return (
                                <span
                                  class={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${condColor}`}
                                >
                                  {condLabel}
                                </span>
                              );
                        ...
                        >
                          <div class="flex items-center gap-4">
                            <p class="text-xs font-medium text-red-700 dark:text-red-300">
                              Permanently delete{" "}
                              <strong>{item.name}</strong>? This cannot be
                              undone.
                            </p>
                            <button
                              type="button"
