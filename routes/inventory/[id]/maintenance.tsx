...
function dueText(
  ...
  const diffDays = Math.floor(
    (nextInspectionDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24),
  );
  if (diffDays < 0) {
    return {
      text: `Inspection overdue by ${Math.abs(diffDays)} day${
...
export const handler: Handlers<MaintenancePageData> = {
  ...
  async POST(req, ctx) {
    ...
    const nextIntervalRaw = String(
      formData.get("nextInspectionIntervalDays") ?? "",
    ).trim();
    const conditionAfterRaw = String(formData.get("conditionAfter") ?? "")
      .trim();

    const allowedTypes: MaintenanceRecord["type"][] = [
      "inspection",
      "repair",
      "cleaning",
      "replacement-part",
    ...
    if (!allowedTypes.includes(typeRaw as MaintenanceRecord["type"])) {
      return ctx.render({
        item,
        error: "Please choose a valid maintenance type.",
        session,
        csrfToken: session.csrfToken,
      });
    }
    if (!notesRaw) {
      return ctx.render({
        item,
        error: "Please add maintenance notes.",
        session,
        csrfToken: session.csrfToken,
      });
    }
    ...
    if (nextIntervalRaw) {
      ...
      if (
        ...
        return ctx.render({
          item,
          error:
            "Next inspection interval must be a whole number between 1 and 3650.",
          session,
          csrfToken: session.csrfToken,
        });
      }
    ...
    await logActivity({
      ...
      details: `Logged ${typeRaw} on ${dateRaw}${
        nextInspectionDate
          ? `, next due ${toDateInputValue(nextInspectionDate)}`
          : ""
      }`,
    });
...
export default function MaintenancePage(
  { data }: PageProps<MaintenancePageData>,
) {
  if (!data.item) {
    return (
      <Layout
            ...
            <div class="rounded-md border border-gray-200 dark:border-gray-700 p-3">
              ...
              <p class="font-semibold text-gray-900 dark:text-gray-100">
                {item.lastInspectedDate
                  ? formatDate(item.lastInspectedDate)
                  : "Not recorded"}
              </p>
            </div>
            <div class="rounded-md border border-gray-200 dark:border-gray-700 p-3">
