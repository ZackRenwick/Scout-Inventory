...
export const handler: Handlers<TemplatesPageData> = {
  ...
  async POST(req, ctx) {
    ...
    if (action === "delete") {
      ...
      if (templateId && itemId) {
        ...
        if (template && inv) {
          ...
          const existingQty = idx >= 0
            ? template.items[idx].quantityPlanned
            : 0;
          ...
          const availableNow = inv.category === "food"
            ...
            : Math.max(
              0,
              inv.quantity - (inv.atCamp
                ? Math.max(
                  0,
                  Math.min(inv.quantity, inv.quantityAtCamp ?? inv.quantity),
                )
                : 0),
            );
    ...
    return new Response(null, {
      status: 303,
      headers: { Location: "/camps/templates" },
    });
  },
};

export default function CampTemplatesPage(
  { data }: PageProps<TemplatesPageData>,
) {
  const isAdmin = data.session?.role === "admin";
  const canEditTemplates = data.session?.role === "admin" ||
    data.session?.role === "editor";
  const csrfToken = data.session?.csrfToken ?? "";

  return (
    <Layout
      title="Camp Equipment Templates"
        ...
        <div class="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p class="mt-1 text-sm text-gray-600 dark:text-gray-400">
              ...
              Create manually below, or open a camp plan and use "📋 Save as
              template".
            </p>
          </div>
          <a
        ...
        ? (
          <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-8 text-center">
            <p class="text-gray-500 dark:text-gray-400 text-sm">
              No templates yet. Use "➕ New Template" above to create one, or
              open a camp plan and use "📋 Save as template".
            </p>
          </div>
        )
        : (
                ...
                {tpl.items.length > 0 && (
                  <details class="mt-3">
                    <summary class="cursor-pointer text-xs text-purple-600 dark:text-purple-400 hover:underline select-none">
                      View {tpl.items.length}{" "}
                      item{tpl.items.length !== 1 ? "s" : ""}
                    </summary>
                      ...
                      {tpl.items.map((item) => (
                        ...
                        >
                          ...
                          <span class="text-gray-500 dark:text-gray-400 text-xs ml-2 shrink-0">
                            {item.itemCategory} · {item.itemLocation} · qty:
                            {" "}
