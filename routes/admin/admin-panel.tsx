...
export const handler = {
  async GET(ctx) {
    ...
    return page({ users, session, csrfToken: session.csrfToken });
  },
  ...
  async POST(ctx) {
    ...
    if (!csrfToken || csrfToken !== session.csrfToken) {
      const users = (await getAllUsers()).map(({ passwordHash: _ph, ...u }) =>
        u
      );
      return page({
        users,
    ...
    >
      <div class="mb-6">
        <p class="text-gray-600 dark:text-gray-400">
          Manage who can access the inventory system
        </p>
      </div>
      ...
      <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6 mb-8">
        ...
        <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
          ...
          LibreOffice, or Google Sheets. All categories and fields are included.
        </p>
        ...
        {isAdmin && (
          <p class="text-xs text-gray-400 dark:text-gray-500 mt-2">
            The JSON file is import-ready — use it directly with the Bulk Import
            below to seed another environment.
          </p>
        )}
      </div>
      ...
      {isAdmin && (
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-1">
            ...
            <strong>.json</strong>{" "}
            file. All items are validated before any are saved — if any row has
            an error, the whole import is rejected.
          </p>
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
      ...
      {isAdmin && (
        <div class="bg-white dark:bg-gray-800 rounded-lg shadow border border-gray-200 dark:border-gray-700 p-6 mb-8">
          ...
          <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">
            Rebuilds all KV secondary indexes and precomputed stats from the
            primary item data. Only needed after a manual data migration or if
          ...
          {users.map((user) => (
            ...
            >
              ...
              <div class="flex flex-wrap items-center gap-2 shrink-0">
                {/* Role change */}
                {user.username !== session.username &&
                  !(session.role === "manager" && user.role === "admin") && (
            ...
            <div>
              ...
              >
                ...
                <option value="manager">
                  Manager — stock-take, exports &amp; users
                </option>
