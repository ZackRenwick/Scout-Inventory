...
function FoodItemCard({ item }: { item: FoodItem }) {
  ...
  return (
    <div
      class={`bg-white dark:bg-gray-900 rounded-lg border border-gray-200 dark:border-gray-700 border-l-4 ${accent} shadow-sm`}
    >
      <div class="flex items-start justify-between px-4 pt-4 pb-2">
        <div class="min-w-0">
          ...
          <div class="text-xs text-gray-500 dark:text-gray-400 capitalize mt-0.5">
            {item.foodType}
          </div>
        </div>
  ...
  return (
    <div>
      <div class={`${headerBg} border-l-4 ${headerBorder} p-4 mb-4`}>
        <h2 class={`text-xl font-bold ${headerText}`}>
          {title} ({items.length})
        </h2>
...
export default function ExpiringFoodPage(
  { data }: PageProps<ExpiringFoodData>,
) {
  const totalItems = data.expired.length + data.expiringSoon.length +
    data.expiringWarning.length + data.fresh.length;

  return (
    <Layout
      title="Expiring Food Report"
      username={data.session?.username}
      role={data.session?.role}
    >
      <div class="mb-6">
        <p class="text-gray-600 dark:text-gray-400">
          Monitor food items approaching their expiry dates
        </p>
      </div>
        ...
        ? (
          <div class="bg-gray-50 dark:bg-gray-800/40 border border-gray-200 dark:border-gray-700 rounded-lg p-8 text-center">
            ...
            <p class="text-gray-500 dark:text-gray-400 mt-2">
              Add food items to start tracking expiry dates.
            </p>
          </div>
        )
        : (
