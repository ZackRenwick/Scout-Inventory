  ...
  async POST(req, ctx) {
    ...
    if (action === "catalog_create") {
      ...
      const section = normalizeSection(
        form.get("section")?.toString() ?? "General",
      );
      if (name) {
      ...
      return new Response(null, {
        status: 303,
        headers: { Location: "/first-aid/catalog" },
      });
    }
...
export default function FirstAidCatalogPage(
  { data }: PageProps<FirstAidCatalogPageData>,
) {
              ...
              <div>
                ...
                >
                  {FIRST_AID_SECTIONS.map((section) => (
                    <option key={`create-section-${section}`} value={section}>
                      {section}
                    </option>
                  ))}
                </select>
                ...
                {group.items.map((item) => (
                  <div key={`catalog-${item.id}`} class="p-3">
                    <div class="grid grid-cols-1 md:grid-cols-12 gap-2 items-end">
                      {canEdit
                        ? (
                          <form
                            method="POST"
                            class="md:col-span-10 grid grid-cols-1 md:grid-cols-10 gap-2 items-end"
                          >
                            <input
                              type="hidden"
                              name="_csrf"
                            ...
                            <div class="md:col-span-6">
                              ...
                              <input
                                name="name"
                                value={item.name}
                                class="w-full px-2 py-1 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-900 text-gray-900 dark:text-gray-100"
                              />
                            </div>
                            ...
                            <button
                              type="submit"
                              class="md:col-span-1 px-2 py-1 bg-gray-100 dark:bg-gray-700 text-gray-800 dark:text-gray-100 rounded hover:bg-gray-200 dark:hover:bg-gray-600"
                            >
                              Save
                            </button>
                          </form>
                        ...
                        <form method="POST" class="md:col-span-2">
                          ...
                          <button
                            ...
                            title={usage.get(item.id)
                              ? `Used in ${usage.get(item.id)} kit${
                                usage.get(item.id) === 1 ? "" : "s"
                              }`
                    ...
                    <p class="mt-1 text-xs text-gray-600 dark:text-gray-300">
                      {usage.get(item.id)
                        ? `Warning: currently used in ${
                          usage.get(item.id)
                        } kit${usage.get(item.id) === 1 ? "" : "s"}.`
                        : "Not currently used in any kit."}
                    </p>
                  </div>
                ))}
