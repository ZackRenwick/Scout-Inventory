...
Add a scan-assisted mode inside an existing camp checklist (`/camps/[id]`) so
leaders can scan multiple item QR labels and build a camp plan quickly.
...
5. Keep scan input focused and show per-scan feedback (`Added`,
   `Already in plan`, `Unknown QR`, `Not available`).
